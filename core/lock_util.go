...
// modeWriteAll is the union of owner/group/other write bits (0o222).
// Spelled as a literal rather than syscall.S_IW* so the package also builds on
// Windows, where those constants are undefined.
const modeWriteAll = os.FileMode(0o222)

// accModeMask is the access-mode portion of open flags (== unix O_ACCMODE).
// Defined locally because syscall.O_ACCMODE is undefined on Windows.
...
func openWantsWrite(openFlags uint32) bool {
	// O_RDONLY is 0 on all platforms; a non-zero access mode means write intent.
	return openFlags&accModeMask != 0
}
